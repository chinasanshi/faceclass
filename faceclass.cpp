...
faceclass::faceclass():_facemodelno(3){}
faceclass::~faceclass(){}
...
{
	if (faceclass::setmodelno(_facemodelno))
	{
		//cout << "加载人脸识别模型前，创建相应的人脸识别模型成功！" << endl;
	}
...
void faceclass::setmodelthreshold(int threshold)
{
	_model->set("threshold", threshold);
	std::cout << "模型阈值设置为" << threshold << std::endl;
}
					...
					for (std::vector<std::vector<std::vector<cv::Rect>>>::iterator itallpedfacerects = _allpedestrianfacerects.begin(); itallpedfacerects != _allpedestrianfacerects.end(); ++itallpedfacerects)
					{
						for (std::vector<std::vector<cv::Rect>>::iterator itpedfacerects = itallpedfacerects->begin(); itpedfacerects != itallpedfacerects->end(); ++itpedfacerects)
						{
							for (std::vector<cv::Rect>::iterator itrects = itpedfacerects->begin(); itrects != itpedfacerects->end(); ++itrects)
							{
								...
								outfile.open("record.txt", ios::app);//以追加的方式写文件
								outfile << "标号：" << predictedLabel << "签到时间" << timetmp << endl;//将签到的人标号和时间记录下来
								outfile.close();
								cout << "标号：" << predictedLabel << "签到时间" << timetmp << endl;;//将签到的人标号和时间输出显示
							}
						}
					}
				}
